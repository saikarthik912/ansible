...
class ExecuteShellScriptAction(Action):
    def run( self, namespace, server_ip, image, secrets):
        try:
            os.chdir("/opt/stackstorm/packs/panoiq_stackstorm/kafka-ui");
            script_path = "/opt/stackstorm/packs/panoiq_stackstorm/kafka-ui/init.sh"
            command = [script_path, namespace, server_ip, image, secrets]
